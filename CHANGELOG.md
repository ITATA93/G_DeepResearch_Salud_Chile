---
depends_on: []
impacts: []
---

# Changelog — G_DeepResearch_Salud_Chile

All notable changes to this project will be documented in this file.
Format follows [Keep a Changelog](https://keepachangelog.com/en/1.1.0/).

## [Unreleased]

- Governance audit: docs/TODO.md verified
- README.md enhanced with architecture and usage docs
- Gemini CLI integration validated

## [1.0.0] — 2026-02-23

### Added
- Full GEN_OS mirror infrastructure migrated from AG_DeepResearch_Salud_Chile.
- Multi-vendor dispatch: .subagents/, .claude/, .codex/, .gemini/, .agent/.
- Governance standards: docs/standards/.
- CI/CD workflows: .github/workflows/.
- All domain content preserved from AG_DeepResearch_Salud_Chile.
